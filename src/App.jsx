...
function FinanceDash({users,students,trainers,plans,onUpdate,setProformaStudent}){
  const now2=new Date();
  ...
  return(
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12,marginBottom:22}}>
        {[{l:"Ingresos netos",v:fmtCLP(totalIngresos),c:"var(--ac)",i:"💵"},{l:"Total c/IVA",v:fmtCLP(totalBruto2),c:"var(--gr)",i:"🏦"},{l:"Costo coaches",v:fmtCLP(totalCostoCoaches),c:"var(--a3)",i:"👨‍🏫"},{l:"Margen bruto",v:fmtCLP(margenNeto),c:margenNeto>=0?"var(--gr)":"var(--a3)",i:"📈"},{l:"Sesiones mes",v:totalSessions,c:"var(--a2)",i:"🏋️"},{l:"Alumnos activos",v:rows.filter(r=>r.sessM>0).length,c:"var(--or)",i:"👥"}].map(x=>(
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              <div>
                ...
                <div style={{fontSize:12,color:"var(--mu)"}}>
                  ...
                  {" "}<span style={{cursor:"pointer",color:"var(--mu)",fontSize:11,textDecoration:"underline"}} onClick={()=>{const nr=prompt(`Tarifa por sesión de ${coach.name} (CLP):`,String(rate));if(nr&&!isNaN(+nr)){onUpdate([...users.filter(u=>u.id!==coach.id),{...coach,sessionRate:+nr}]);}}}>✏️ editar</span>
                </div>
              </div>
              <div style={{textAlign:"right"}}>
...
function Login({users,onLogin,onUpdateUsers}){
  const[screen,setScreen]=useState("login"); // login | forgot | verify | newpass
